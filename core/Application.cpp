...
{
    if(Hagman.gameState == Gamming)
    {
        Hagman.StartTimer();
        Hagman.handleEvent(ev);
    }
    else
    {
        Hagman.PauseTimer();
...
void Application::handleInputBuffers(const sf::Vector2i& delta_mp)
{
    ...
    if (button_buffer_[sf::Mouse::Left])
    {
        ...
        if (key_buffer_[sf::Keyboard::LControl] && app_state_ == Gamming)
        {
        ...
        else if (key_buffer_[sf::Keyboard::LShift] && app_state_ == Gamming)
        {
